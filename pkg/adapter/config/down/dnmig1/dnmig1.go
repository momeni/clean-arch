...
// Package dnmig1 provides configuration file settings downwards
// Migrator for settings major version 1 and its Adapter type for the
...
// version 1) as C and the parameterized settings.DownMigrator interface
...
type (
	...
	Type = settings.DownMigrator[C, S, *Migrator]
)
...
// the Adapt function.
// Since major version 1 has no downer version, this method always
// returns an error.
...
// Migrator is a downwards Config migrator for *cfg1.Config instances.
...
// MigrateDown should migrate the wrapped cfg1.Config instance to its
