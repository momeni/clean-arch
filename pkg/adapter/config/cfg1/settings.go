...
type OutOfBoundsSettingsError struct {
	...
	Cars struct {
		// OldParkingDelay indicates the range violation error (if any)
		// with regards to the old parking method delay.
		OldParkingDelay *settings.OutOfRangeError[settings.Duration]
	}
}
...
func (e *OutOfBoundsSettingsError) Error() string {
	return fmt.Sprintf("cfg1.Config settings are out of bounds: %#v", e)
}
...
func (c *Config) Mutate(s Serializable) error {
	...
	if err := settings.VerifyRange(
		...
		boundsErr.Cars.OldParkingDelay = err
		hasBoundsErr = true
	}
	if hasBoundsErr {
		return boundsErr
...
func (c *Config) Bounds() (minb, maxb *Serializable) {
	...
	settings.OverwriteUnconditionally(
		&minb.Settings.Visible.Cars.OldParkingDelay,
		c.Usecases.Cars.MinOldParkingDelay,
	)
	maxb = &Serializable{
		Version: c.Version(),
	...
	settings.OverwriteUnconditionally(
		&maxb.Settings.Visible.Cars.OldParkingDelay,
		c.Usecases.Cars.MaxOldParkingDelay,
	)
	return minb, maxb
}
