...
// Package sch1v0 provides the top-level Migrator type for database
// schema version 1.0.x which can be used for starting a multi-database
// migration operation. This package contains the main logic for
// querying of v1.0 schema and converting them to the latest supported
...
// high-level database schema migration logic for the v1.0 schema.
...
// source database schema (having v1.0 format) accessible from the
...
// remote tables in a schema such as fdw1_0 while the settler object
// expects a schema such as mig1 for its data persistence queries.
func (s1v0 *Migrator) Settler(
	ctx context.Context,
) (*stlmig1.Settler, error) {
	if err := s1v0.migrateToLastMinorVersion(ctx); err != nil {
		return nil, err
	}
	return stlmig1.New(s1v0.tx), nil
}
...
// schema using a series of views, converting the v1.0 to the v1.1
// schema. Finally, it returns an instance of the upwards migrator
// object (having the U type) which can be used to migrate schema to
// the next major versions (if any) or obtain the settler object.
func (s1v0 *Migrator) UpMigrator(
	ctx context.Context,
) (*upmig1.Migrator, error) {
	if err := s1v0.migrateToLastMinorVersion(ctx); err != nil {
		return nil, err
	}
	return &upmig1.Migrator{s1v0.tx}, nil
}

// DownMigrator expects the fdw1_0 schema to contain the source database
// contents (created by the Load method) and it fills the mig1 local
// schema using a series of views, converting the v1.0 to the v1.1
// schema. Finally, it returns an instance of the downwards migrator
// object (having the D type) which can be used to migrate schema to
// the previous major versions (if any) or obtain the settler object.
func (s1v0 *Migrator) DownMigrator(
	ctx context.Context,
